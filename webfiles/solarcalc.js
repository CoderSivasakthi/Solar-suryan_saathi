...
let baseEnergyConsumed = 0;

// Function to update the displayed energy consumed
function updateEnergyConsumed() {
    let consumptionMultiplier = parseInt(numberInput.value);
    evalDisplay.textContent = `${baseEnergyConsumed + consumptionMultiplier} KW`;
}

// Event listeners for the decrement button
decrementButton.addEventListener('mousedown', () => {
    if (parseInt(numberInput.value) > 0) {
        numberInput.value = parseInt(numberInput.value) - 1;
        updateEnergyConsumed();  // Update the energy consumed display
    }
});

// Event listeners for the increment button
incrementButton.addEventListener('mousedown', () => {
    numberInput.value = parseInt(numberInput.value) + 1;
    updateEnergyConsumed();  // Update the energy consumed display
});

// Ensure that changes to the input field also trigger an update
